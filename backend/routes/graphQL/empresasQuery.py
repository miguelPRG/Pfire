...
class EmpresaQuery:
    ...
    async def getEmpresas(
        self, info: Info, id: str = None, start: int = 0, filter: EmpresaFilter = None
    ) -> EmpresaList:
        ...
        if id:
            ...
            if filter.nome:
                ...
                filtro_emp["localidade"] = {
                    "$regex": f"^{filter.localidade}",
                    "$options": "i",
                }
            elif filter.morada:
                filtro_emp["morada"] = {"$regex": f"^{filter.morada}", "$options": "i"}
            elif filter.codigo_postal:
                filtro_emp["codigo_postal"] = {
                    "$regex": f"^{filter.codigo_postal}",
                    "$options": "i",
                }
            elif filter.telefone:
                filtro_emp["telefone"] = {
                    "$regex": f"{filter.telefone}",
                    "$options": "i",
                }

            total_empresas = await empresas_collection.count_documents(filtro_emp)
            async for empresa in (
                empresas_collection.find(filtro_emp).skip(start).limit(lmt)
            ):
                # Verifica permissões
                if jwt.get("isSuperAdmin", False):
                    is_admin = True
        ...
        # Caso contrário, retorna empresas associadas ao utilizador
            ...
            user_empresas = await users_empresas_collection.find(
                {"user_id": ObjectId(jwt["user_id"])}
            ).to_list(None)
        ...
        async for empresa in empresas_collection.find(filtro).skip(start).limit(lmt):
            ...
            if jwt.get("isSuperAdmin", False):
                ...
                empresa_data["isAdmin"] = any(
                    user_empresa["empresa_id"] == empresa["_id"]
                    and user_empresa.get("isAdmin", False)
