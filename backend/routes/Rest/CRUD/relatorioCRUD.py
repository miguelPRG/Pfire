            ...
            if value["datatype"] == "object":
                ...
                for k in relatorio_fields[key].keys():
                    if not k.startswith("custom_"):
                        raise HTTPException(status_code=400, detail=f"O campo {full_key} não pode conter subcampos que não começam com 'custom_'.")
                    else:
                        custom_fields[k] = relatorio_fields[key][k]

                if not custom_fields:
                    raise HTTPException(status_code=400, detail=f"O campo {full_key} deve conter pelo menos um subcampo personalizado (custom_).")
